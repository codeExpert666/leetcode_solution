...
// 细节: 利用按位异或的性质，可以得到 mid 和相邻的数之间的如下关系，其中 ⊕ 是按位异或运算符：
...
// 得到下标 x 的值之后，nums[x] 即为只出现一次的元素。
// 细节: 考虑 mid 和 1 按位与运算的结果，其中 & 是按位与运算符：
// 当 mid 是偶数时，mid & 1=0；
// 当 mid 是奇数时，mid & 1=1。
...
// 方法三：异或。虽不满足题目要求的时间复杂度，但也是一个好方法
// 举个例子好理解：a a b c c
// 0⊕a=a, a⊕a=0, 0⊕b=b, b⊕c⊕c=b
// 所以不停顺序异或，最终保留下的就是单一元素

// 方法四：一次遍历，找到第一个不满足num[i]==nums[i+1]的偶数下标，该位置处即为单一元素
